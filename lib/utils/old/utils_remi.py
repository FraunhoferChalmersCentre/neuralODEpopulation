...
def plot_individual_fits(iteration_for,global_max_dose,global_max_time,
 ...
 # Determine number of groups
    ...
    for i, idx in enumerate(selected_indices):
        ...
        subject_id, t, x, mask, t_encoder, x_encoder, \
        dose_tensor, dose_times_tensor, dose_times_mask, transfusion_times_expanded = entry
        ...
        x_low_normalized = x_encoder.to(device)
   ...
   # Compute sums only on masked points
       ...
       #_, mse = noise.nll(y_true, pred_interp)
        ...
        ax.set_title(f"ID | Dose {global_max_dose * dose_val:.2f}")
  ...
  #      id_list.append(np.array([s] * len(y_true_masked.cpu())))
...
def prepare_datasets_and_loaders(data_path,base_dir, all_ids, i,already_done, global_max_dose, global_max_time,
  ...
  #  export_training_data(test_dataset, train_export_dataset, global_mean, global_std, global_max_time, i+already_done, trunctation, base_dir)
    ...
    val_loader   = DataLoader(val_dataset, batch_size=2*len(val_ids), shuffle=False, collate_fn=collate_fn, drop_last=True)
    test_loader  = DataLoader(test_dataset, batch_size=2*len(test_ids), shuffle=False, collate_fn=collate_fn, drop_last=True)
...
def run_model_variant(variant_name, train_dataset, val_dataset, test_dataset,
    ...
    mse_train, mse_validation = train_loop_model(
        ...
        smoothing_start_epoch=1000, traing_against_validation=False,
        ...
        free_bits=free_bits, max_points_visible=0.6, print_epoch=1,
...
dose_tensor_expanded, dose_times_expanded, dose_times_mask, transfusion_times_expanded = preprocess_batch(
...
def preprocess_batch(batch, device, max_points_visible=0):
    occ_list, t_padded, x_padded, mask, dose_tensor, dose_times_list, tinfcat_tensor = batch

    t_padded, x_padded, mask, tinfcat_tensor = (
        t_padded.to(device), x_padded.to(device), mask.to(device), tinfcat_tensor.to(device)
    )
    ...
    dose_times_expanded = dose_times_padded.unsqueeze(-1)
    transfusion_times_expanded = tinfcat_tensor.unsqueeze(1).unsqueeze(-1)
  #  print("t:", t)


    if max_points_visible > 0:
        t_list, x_list, _ = truncate_time_series(t_padded, x_padded, max_points_visible)
        t_encoder = pad_sequence(t_list, batch_first=True).to(device)
        x_encoder = pad_sequence(x_list, batch_first=True).to(device)
    else:
        t_encoder, x_encoder = t_padded, x_padded
...
def prepare_ode_input(
    ...
    func,
    dose_times_expanded,
    transfusion_times_expanded,  # <-- new argument,
    dose_tensor_expanded,
    dose_times_mask
    ...
    # Create dose mask: 1 where dose exists, 0 otherwise
    dose_mask = (dose_tensor_expanded != 0).float().unsqueeze(-1)  # [batch_size, num_doses, 1]

    # Pass dose_times, dose_amounts, dose_mask, and transfusion durations to ODEWrapper
    ode_func = ODEWrapper(
    func,
    dose_times_expanded,
    transfusion_times_expanded,
    ...
    for epoch in range(n_epochs):
        ...
        for batch in dataloader:
            ...
            (
             ...
             tinfcat_tensor    # transfusion durations (TINFCAT)
    ...
    dose_tensor_expanded, dose_times_expanded, dose_times_mask, transfusion_times_expanded = preprocess_batch(
        batch, device, max_points_visible=max_points_visible
            ...
            x0, ode_func = prepare_ode_input(
                ...
                dose_times_expanded,transfusion_times_expanded,  dose_tensor_expanded, dose_times_mask
            )
        ...
        if plot_from_training_records_enable and epoch % plot_epoch == 0:
            ...
            plot_individual_predictions(t_dense, models, dataset, device, global_mean, global_std, global_max_time,
                                        max_points_visible=100, max_plots=25, nr_row=5, nr_col=5)
...
def plot_individual_predictions(latent_dim, t_dense, models, dataset, device, global_mean, global_std, global_max_time,
    ...
    for i, data in enumerate(dataloader):
        ...
        # Unpack batch
        id_list, t_padded, x_global_padded, mask,dose_tensor, dose_times_list, \
            tinfcat_tensor = data
        
        
        # Move to device
        t_padded = t_padded.to(device)
        x_padded = x_global_padded.to(device)
        mask = mask.to(device)
        dose_tensor = dose_tensor.to(device)
        tinfcat_tensor = tinfcat_tensor.to(device)
        

        # Ensure 2D dose tensors
        if dose_tensor.dim() == 1:
            dose_tensor = dose_tensor.unsqueeze(0)  # [1] -> [1, 1]
        if tinfcat_tensor.dim() == 1:
            tinfcat_tensor = tinfcat_tensor.unsqueeze(0)

        # Convert dose_times_list (list of lists) into padded tensor + mask
        batch_size = len(dose_times_list)
        max_doses = max(len(d) for d in dose_times_list)
        dose_times_tensor = torch.zeros((batch_size, max_doses), device=device)
        dose_mask = torch.zeros((batch_size, max_doses, 1), device=device)
        for j, d in enumerate(dose_times_list):
            l = len(d)
            if l > 0:
                dose_times_tensor[j, :l] = torch.tensor(d, device=device)
                dose_mask[j, :l, 0] = 1.0  # mark valid doses
        ...
        # Prepare ODE input
        x0, ode_func = prepare_ode_input(
            initial_encoder, x_padded, z_refined,
            func, dose_times_tensor, tinfcat_tensor, dose_tensor, dose_mask
        )

        # Generate predictions (ODE solver)
        with torch.no_grad():
            pred = odeint(ode_func, x0, t_dense, method='rk4')
            pred_batch = pred.permute(1, 0, 2)
            pred = reducer(pred_batch[:, :, :latent_dim])

        # Denormalize and squeeze to 1D
        pred_plot = pred.squeeze(0).cpu() * global_std + global_mean  # [T]
        x_true_plot = x_padded.squeeze(0)[:mask.sum()].cpu() * global_std + global_mean  # [T]
        t_plot = t_padded.squeeze(0)[:mask.sum()].cpu()  # [T]
        
        # Ensure chronological order
        t_sorted, sort_idx = torch.sort(t_plot)
        x_true_plot = x_true_plot[sort_idx]
        pred_plot = pred_plot[sort_idx]
        tinfcat_val = tinfcat_tensor.item() #* global_max_time  # convert normalized TINFCAT to actual time

        # Plot
        ax = axes[i]
        ax.plot(t_sorted, x_true_plot, 'o', label='Actual')
        ax.plot(t_sorted, pred_plot, '-', label='Predicted')
        ax.set_title(f'Individual {i} | TINFCAT = {tinfcat_val:.2f}')
...
class TrajectoryDataset(Dataset):
    def __init__(
        ...
        conc_mean=None,
        conc_std=None,
        sex_max_abs_dict=None,
        subset_ids=None  # <-- new argument
    ):
        # === Step 1: Load data ===
        ...
        self.df['DV_norm'] = (self.df[self.compartment] - self.conc_mean) / self.conc_std

        # === Step 3: Per-Sex median normalization ===
        # if sex_max_abs_dict is None:
        #     sex_group_median = {}
        #     for sex, df_sex in self.df.groupby('SEX'):
        #         median_val = float(df_sex[self.compartment].median())
        #         sex_group_median[sex] = median_val if median_val != 0 else 1.0
        #     self.sex_max_abs = sex_group_median
        # else:
        #     self.sex_max_abs = sex_max_abs_dict

        # self.df['DV_sex_norm'] = self.df.apply(
        #     lambda row: row[self.compartment] / (self.sex_max_abs[row['SEX']] + 1e-8),
        #     axis=1
        # )

        # === Step 4: Dose times placeholder ===
        self.df['DOSE TIME'] = self.df['DOSE TIME'].apply(ast.literal_eval)
        self.df['DOSE TIME'] = self.df['DOSE TIME'].apply(
            lambda lst: [float(t) for t in lst]  # force into list of floats
        )
    ...
    def _extract_trajectories(self):
        start_idxs = self.df[self.df['TIME'] == 4.5].index.tolist() + [len(self.df)]
        trajectories = []
    
        for subject_id, group in self.df.groupby('ID'):
           
    
            # Transfusion time / infusion duration normalized
            tinfcat = torch.tensor(group['TINFCAT'].values[0], dtype=torch.float32) / self.max_time  
    
            # Times and observations
            t = torch.tensor(group['TIME'].values, dtype=torch.float32) / self.max_time  # normalize
            x_global = torch.tensor(group['DV_norm'].values, dtype=torch.float32)
            #x_sex = torch.tensor([r['DV_sex_norm'] for r in rows], dtype=torch.float32)
    
            # Dose amounts normalized
            amt = torch.tensor(group['AMT'].values[5], dtype=torch.float32) / self.max_dose
            
            # Dose times normalized
            dose_times = torch.tensor(group['DOSE TIME'].values[0], dtype=torch.float32) / self.max_time
            ...
            trajectories.append({
                ...
                'TINFCAT': tinfcat,        # normalized infusion duration
    ...
    def _generate_samples(self, trajectories):
        ...
        for traj in trajectories:
            ...
            tinfcat = traj['TINFCAT']          # <-- grab TINFCAT
          ...
          #  rows = traj['rows']
            ...
            augmented.append({
                ...
                'amt': amt,
                'dose_times': dose_times,
                'TINFCAT': tinfcat,           # <-- include here
                'subject_id': subject_id_str,
               # 'x_sex': x_sex,
             #   'rows': rows
            })
    
            # Optional prefix augmentation
            if self.augment:
                T = len(t)
                for end in range(1, T):
                    augmented.append({
                        ...
                        'amt': amt,
                        'dose_times': dose_times,
                        'TINFCAT': tinfcat,       # <-- include here too
...
def collate_fn(batch):
    ...
    tinfcat_list = [s['TINFCAT'] for s in batch]  # <-- new
    id_list = [s['subject_id'] for s in batch]
  #  x_dose_list = [s['x_sex'] for s in batch]
   # rows_list = [s['rows'] for s in batch]  # all original columns, optional
  ...
  #  x_dose_padded = pad_sequence(x_dose_list, batch_first=True)
    ...
    tinfcat_tensor = torch.stack(tinfcat_list)  # <-- new
