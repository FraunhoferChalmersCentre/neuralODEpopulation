...
def export_training_data(train_dataset, test_dataset, global_mean, global_std, global_max_time, i, base_dir, truncation):
    """
    Export training and test data from TrajectoryDataset into Monolix-compatible CSV.
    """
    def flatten_samples(samples, censoring_flag=0, truncate_mask=None):
        """Flatten dataset samples into lists for DataFrame export."""
        flat_ids, flat_t, flat_x, flat_time0, censoring = [], [], [], [], []
        ...
        for s in samples:
            ...
            flat_ids.extend([subj_id] * len(t_list))
            # Mark dosing event (example: time==3 after rescaling)
    ...
    # --- TRAINING DATA ---
    ids_train, t_train, x_train, time0_train, censor_train = flatten_samples(train_dataset)

    # --- TEST DATA (apply truncation) ---
    test_tensors = [s["t"] for s in test_dataset]
    test_values = [s["x_global"] for s in test_dataset]
    test_ids = [s["subject_id"] for s in test_dataset]

    truncation_time = truncation
    truncated_t, truncated_x, masks, _, _ = truncate_time_series(test_tensors, test_values, truncation_time)

    # rebuild truncated samples with IDs
    truncated_samples = [
        {"t": t, "x_global": x, "subject_id": sid}
        for t, x, sid in zip(truncated_t, truncated_x, test_ids)
    ]
    ids_test, t_test, x_test, time0_test, censor_test = flatten_samples(truncated_samples)

    # --- DISCARDED (censored) TEST POINTS ---
    disc_t, disc_x, disc_ids, time0_disc, censor_disc = [], [], [], [], []
    for t_i, x_i, mask, sid in zip(test_tensors, test_values, masks, test_ids):
        inv_mask = ~mask.bool()
        discarded = [{"t": t_i[inv_mask], "x_global": x_i[inv_mask], "subject_id": sid}]
        ids_d, t_d, x_d, time0_d, censor_d = flatten_samples(discarded, censoring_flag=1)
...
def prepare_datasets_and_loaders(data_path,base_dir, all_ids, i,already_done, device, batch_fraction=0.05,truncation=1):
   ...
   # all_ids = list(range(n_ids))
    ...
    # Ensure the IDs column exists; assuming your dataset has an 'ID' column
    if 'ID' not in df.columns:
        df['ID'] = range(len(df))  # fallback: create an ID column if missing

    # Split the dataset using train/val/test IDs
    df_train = df[df['ID'].isin(train_ids)].reset_index(drop=True)
    
    global_max_dose, global_max_time, global_mean, global_std, global_max_value = compute_global_stats(df_train)
   ...
   # val_loader = DataLoader(val_dataset, batch_size=1, shuffle=False, collate_fn=collate_fn)
    ...
    return train_dataset, test_dataset, train_loader,test_loader, combined, global_max_dose, global_max_time, global_mean, global_std, global_max_value
...
def prepare_datasets_and_loaders_train(data_path,base_dir, all_ids, device, batch_fraction=0.05,truncation=1):
   ...
   # all_ids = list(range(n_ids))
    ...
    # Compute exact counts
    n_test = 0 # int(12 * 0.3)   # 3
    n_val  = 0 #int(12 * 0.1)   # 1
    n_train =  12 #12 - n_test - n_val  # 8
   ...
   # val_loader = DataLoader(val_dataset, batch_size=1, shuffle=False, collate_fn=collate_fn)
    ...
    return train_dataset, train_loader, combined, global_max_dose, global_max_time, global_mean, global_std, global_max_value
