...
if __name__ == "__main__":
    ...
    from lib.utils.utils_preprocess import prepare_optimizer, prepare_datasets_and_loaders_simulated, compute_global_stats, export_all_metrics_and_residuals, append_metrics, load_all_metrics_and_residuals_as_lists, TrajectoryDataset, collate_fn
    from lib.utils.utils_training import train_loop_model, vpc, plot_individual_predictions, run_model_variant, ODEWrapper
  ...
  #  device = torch.device("cpu")
    ...
    encoder_vae = Encoder_Transformer_NF(dim_parameter_encoder, input_dim=2, model_dim=64,hidden_dim=64,hidden_flow_dim=16, num_heads=4, num_layers=2,num_flow_layers=3, dropout=0.1).to(device)
    ...
    noise_vae = TrainableNoise(size=1, init_add_std=2, init_prop_std=0).to(device)
    #3.18
    models = {
    "func": func_vae,
    "encoder": encoder_vae,
...
# Post-training:
# -----------------------
plot_individual_predictions(latent_dim, combined, models, dataset_train, device, global_mean, global_std, global_max_time,
                                enable_nf_training=False, enable_ae_training=True, enable_onlymedian_training=False, truncation=1, max_plots=25, nr_row=5, nr_col=5)
